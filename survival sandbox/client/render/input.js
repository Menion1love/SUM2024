...
function setcol () {
  ...
  let img  = new Image(), img1 = new Image();
...
export function inputResponse(rnd, IsMove) {
    ...
    // Count land collisions 
      for (let i = 0; i < 22; i++) {
        if (colmap[1800 - Math.round(rnd.centerY) - i][Math.round(rnd.centerX)] == 1)
          up = false
        if (colmap[1800 - Math.round(rnd.centerY) + i][Math.round(rnd.centerX)] == 1)
          down = false
        if (colmap[1800 - Math.round(rnd.centerY)][Math.round(rnd.centerX) - i] == 1)
          left = false
        if (colmap[1800 - Math.round(rnd.centerY)][Math.round(rnd.centerX) + i] == 1)
          right = false
